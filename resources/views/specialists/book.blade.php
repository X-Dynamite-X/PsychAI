...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex items-center mb-6">
            <img src="{{ $specialist->image }}" alt="{{ $specialist->name }}" class="w-20 h-20 rounded-full object-cover">
            <div class="mr-4">
                <h1 class="text-2xl font-bold text-gray-800">حجز موعد مع {{ $specialist->name }}</h1>
                <p class="text-gray-600">{{ $specialist->title }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('specialists.storeBooking', $specialist) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select id="time" 
                        ...
                        class="w-full border-2 border-gray-300 p-3 rounded-lg focus:outline-none focus:border-green-500"
                        required>
                    <option value="">اختر وقت<|im_start|> مناسباً</option>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <p class="text-gray-600">{{ $specialist->Cost }} ريال</p>
            </div>
